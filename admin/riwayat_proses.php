...
//koneksi database
$hasil = mysqli_query($koneksi,"select * from tb_absen WHERE status='proses'");
...
<nav class="nav nav-borders">
        <a class="nav-link text-primary" href="riwayat.php" target="__blank">Riwayat Persensi</a>
    </nav>
    <hr class="mt-0 mb-4">
    <div class="card">
        <br>
  ...
  <div>
   ...
   <input type="search" class="form-control light-table-filter" style="width:200px" data-table="table-striped" placeholder="Mencari..." />
    ...
    ?>
            <tr>
            <th scope="row"><?=$z++?></th>
            <th scope="row"><img class="img-t" src="upload/<?= $tampil["img"];?>"></th>
            ...
            <th scope="row">
              <a href="pro_selesai.php?id=<?=$tampil['id_absen']?>">
                 <button type="button" class="btn btn-warning text-light fw-bold" onclick="return confirm('Yakin Siswa Persensi Dengan Benar ?')"><?= $tampil["status"];?></button>
              </a>
            </th>
            <!-- <td>
